...
import useCaregivingProgressMessageStatusList from '../../hooks/api/message/use-caregiving-progress-message-status-list'
import usePagination from '../../hooks/use-pagination'
import useCaregivingProgressMessageCreate from '../../hooks/api/message/use-caregiving-progress-message-create'
...
import MessageStatusesCaregivingProgressView from '~views/message-statuses-caregiving-progress'

const MessageStatusesCaregivingProgressPage: NextPage = observer(() => {
  ...
  const [searchFilterStore] = useState(
    () =>
      new SearchFilterStore<MessageStatusesCaregivingProgressPageSearchFilterKey>(
        {
  ...
  const caregivingProgressMessageStatusListData =
    useCaregivingProgressMessageStatusList({
      date: searchFilterStore.searchFilter.DATE,
      onSuccess: (data) => {
        setSelectedItems(data.items.map((item) => item.caregivingRoundId))
      },
      pageNumber,
  ...
  const {mutate: createCaregivingProgressMessage} =
    useCaregivingProgressMessageCreate()
  ...
  const handleOnClickSendMessage = useCallback(() => {
    ...
    const requestPayload: ICaregivingProgressMessageCreate[] =
      selectedItems.map((item) => ({caregivingRoundId: item}))

    createCaregivingProgressMessage(
      {
      ...
      },
      {
        onSuccess: () => {
          queryClient.invalidateQueries({
            queryKey: ['caregiving-progress-message-status', 'list'],
          })
        },
      },
    )
  }, [createCaregivingProgressMessage, queryClient, selectedItems])
  ...
  const handleOnChangeSearchFilter =
    <K extends MessageStatusesCaregivingProgressPageSearchFilterKey>(key: K) =>
  ...
  return (
    <Layout currentPage="NOTIFICATION">
      {caregivingProgressMessageStatusListData ? (
        <MessageStatusesCaregivingProgressView
          data={caregivingProgressMessageStatusListData}
...
export default MessageStatusesCaregivingProgressPage
